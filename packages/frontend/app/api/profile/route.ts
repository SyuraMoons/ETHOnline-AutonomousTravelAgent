...
// this route only ever calls it with no arguments to read the current session.
...
export async function GET() {
  const email = getEmail(await getSession());
...
export async function PATCH(req: NextRequest) {
  ...
  const payload = (await req.json().catch(() => null)) as ProfilePayload | null;
