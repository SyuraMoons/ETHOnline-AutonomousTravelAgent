/**
 * One-time setup: creates the real HCS topic the audit trail writes to
 * (services/hedera/hcsAudit.ts) and prints its id to paste into HCS_AUDIT_TOPIC_ID.
 *
 * Submit key = the agent's own key, so only this app can write events; Mirror Node reads stay
 * public regardless of the submit key.
 *
 * Usage:  npx tsx scripts/hcs-create-audit-topic.ts
...
async function main() {
  ...
  const receipt = await (
    await new TopicCreateTransaction().setSubmitKey(privateKey.publicKey).setTopicMemo("Rayban audit trail").execute(client)
  ).getReceipt(client);

  const topicId = receipt.topicId;
  if (!topicId) throw new Error("TopicCreateTransaction receipt did not include a topicId");

  console.log("HCS_AUDIT_TOPIC_ID (put this in packages/frontend/.env):");
