...
async function main() {
  ...
  if (names.length === 0) {
    throw new Error(
      `No deployments found for "${hre.network.name}". Run \`npm run hardhat:deploy --network ${hre.network.name}\` first.`,
    );
  }
  ...
  if (verified === 0) {
    throw new Error(
      `No verifiable deployments on "${hre.network.name}". Deploy a contract first, or remove stale records under deployments/.`,
    );
  }
}
