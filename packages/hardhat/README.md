...
4. **Verify on Hashscan** (reads `deployments/<network>/` and submits to Sourcify):
   ```bash
   npm run hardhat:verify:testnet   # chain 296
   npm run hardhat:verify:mainnet   # chain 295
   ```
   Requires a prior deploy on that network so `deployments/<network>/<YourContract>.json` exists.
   Stale deployment JSON from other templates is skipped automatically.

   To verify a single address manually:
   ```bash
   npm run verify -w @sh/hardhat -- --network hederaTestnet 0xYourContractAddress
   ```
